...
import { OffsetService } from "src/app/shared/services/offset.service";
...
export class CanvasMainComponent implements OnInit {
  ...
  async ngOnInit() {
    this.psuedoPreLoadMode(this.img_sources);
  }
  ngAfterViewInit() {
    this.initGame();
  }
  ...
  /* very small graphics */
  psuedoPreLoadMode(sources: any): any {
    this.modalService.open({
      popupComponent: PopupComponent,
      data: { 'title':'Chess', 'message': 'i\'m loading your board ...' }
    });
  ...
  mouseClick(): void {
    ...
    this.canvas.onmousedown = (e: {
      ...
      let entry = this.getPawnByMousePosition((e.clientX - this.offsetLeft), 
  ...
  setOffset():void{
    const offset = this.offsetService.getOffset(this.context);
    this.offsetLeft = Math.round(offset.left);
    this.offsetTop = Math.round(offset.top);
  }
